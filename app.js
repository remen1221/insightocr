(() => {
  // STAGE 1だけ画面に目標実数を表示。STAGE 2/3は内部判定のみ。
  const STAGE_TARGETS = {
  ...
  function targetForCurrent() {
    const base = STAGE_TARGETS[state.stage];
  ...
  function updateTargetBox() {
    ...
    if (state.stage === 1) {
      const comboNote = state.mode === 'duo' ? '（コンビ目標は1.5倍）' : '';
      els.targetBox.innerHTML = `<strong>STAGE 1 の目標 ${comboNote}</strong><br>配信回数：${t.count}回　／　トータルリスナー：${t.listeners.toLocaleString()}人　／　配信時間：${fmtMinutes(t.minutes)}`;
    } else {
      els.targetBox.innerHTML = `<strong>STAGE ${state.stage}</strong><br>目標実数は非公開です。画像を集計すると「CLEAR / 未達成」のみ判定します。`;
    }
  }
  ...
  function renderPublicStage1(total, t) {
    const metrics=[
    ...
    const clear = total.usable>0 && total.count>=t.count && total.listeners>=t.listeners && total.minutes>=t.minutes;
    return clear;
  }

  function renderSecretTotals(total) {
    els.totals.innerHTML = [
      ['配信回数', `${Number(total.count).toLocaleString()}回`],
      ['リスナー', `${Number(total.listeners).toLocaleString()}人`],
      ['配信時間', fmtMinutes(total.minutes)],
    ].map(([name,val]) => `<div class="metric"><div class="name">${name}</div><div class="value">${val}</div><div class="sub">今回アップロード分の合計</div></div>`).join('');
  }

  let recalcSeq = 0;
  async function recalc() {
    const seq = ++recalcSeq;
    const a=summarize(state.A), b=summarize(state.B);
    ...
    if (!enough) {
      els.totals.innerHTML='';
      els.verdict.className='verdict waiting';
      els.verdict.textContent='画像を追加し、読み取り結果を確認してください。';
      els.copyBtn.disabled=true;
      state.lastSummary={total,clear:false,enough:false,a,b};
      return;
    }

    if (state.stage === 1) {
      const t = targetForCurrent();
      const clear = renderPublicStage1(total,t);
      if(clear){ els.verdict.className='verdict clear'; els.verdict.innerHTML=`🎉 STAGE 1 CLEAR！<br><span style="font-size:12px;font-weight:700">運営陣の最終確認後、次へ進めます。</span>`; }
      else { els.verdict.className='verdict notyet'; els.verdict.innerHTML='あと少し！ STAGE 1 はまだ未達成です。'; }
      els.copyBtn.disabled=false;
      state.lastSummary={total,target:t,clear,enough:true,a,b,secret:false};
      return;
    }

    renderSecretTotals(total);
    const t = targetForCurrent();
    const clear = total.count>=t.count && total.listeners>=t.listeners && total.minutes>=t.minutes;
    if(clear){
      els.verdict.className='verdict clear';
      els.verdict.innerHTML=`🎉 STAGE ${state.stage} CLEAR！<br><span style="font-size:12px;font-weight:700">目標実数は非公開です。運営陣の最終確認後、次へ進めます。</span>`;
    } else {
      els.verdict.className='verdict notyet';
      els.verdict.innerHTML=`STAGE ${state.stage} はまだ未達成です。<br><span style="font-size:12px;font-weight:700">目標実数・残り数値は非公開です。</span>`;
    }
    els.copyBtn.disabled=false;
    state.lastSummary={total,clear,enough:true,a,b,secret:true};
  }
  ...
  async function copyResult(){
    ...
    const x=s.total;
    let lines=[`【STAGE ${state.stage} 判定｜${state.mode==='duo'?'コンビ':'ソロ'}】`];
    if (!s.secret) {
      const t=s.target;
      ...
      lines.push(line('配信回数',x.count,t.count,'回'), line('トータルリスナー',x.listeners,t.listeners,'人'), line('配信時間',x.minutes,t.minutes,'min'));
    } else {
      lines.push(`配信回数：${Number(x.count).toLocaleString()}回`, `トータルリスナー：${Number(x.listeners).toLocaleString()}人`, `配信時間：${fmtMinutes(x.minutes)}`);
    }
    lines.push('', s.clear?`🎉 STAGE ${state.stage} CLEAR！`:`STAGE ${state.stage} はまだ未達成です。`);
    if (s.secret) lines.push('※STAGE 2・3の目標実数・残り数値は非公開です。');
    lines.push('※自動判定のため、最終確認は運営陣が行います。');
